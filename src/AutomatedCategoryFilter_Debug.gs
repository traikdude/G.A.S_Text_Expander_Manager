...
/**
 * Configuration object for category filtering system (DEBUG VERSION)
 */
const CATEGORY_FILTER_CONFIG_DEBUG = {
...
/**
 * Custom error class for category filtering operations (DEBUG VERSION)
 */
class CategoryFilterErrorDebug extends Error {
  constructor(message, code = 'UNKNOWN_ERROR') {
    super(message);
    this.name = 'CategoryFilterErrorDebug';
    this.code = code;
...
class CategoryFilterManagerDebug {
  ...
  constructor() {
    this.spreadsheet = SpreadsheetApp.openById(CATEGORY_FILTER_CONFIG_DEBUG.SPREADSHEET_ID);
    this.cache = CacheService.getScriptCache();
    this.logger = new CategorySessionLoggerDebug('CategoryFilterManagerDebug');
  }
  ...
  getMainCategories() {
    try {
      ...
      // Try cache first
      const cacheKey = 'main_categories_debug';
      ...
      // Fetch from sheet
      const sheet = this.spreadsheet.getSheetByName(CATEGORY_FILTER_CONFIG_DEBUG.SHEETS.MAIN);
      if (!sheet) {
        throw new CategoryFilterErrorDebug('Main sheet not found', 'SHEET_NOT_FOUND');
      }
      ...
      // Cache results
      this.cache.put(cacheKey, JSON.stringify(uniqueCategories), CATEGORY_FILTER_CONFIG_DEBUG.CACHE_DURATION);
      ...
      throw new CategoryFilterErrorDebug(`Failed to get main categories: ${error.message}`, 'GET_CATEGORIES_FAILED');
    }
  }
  ...
  getSubcategories(mainCategory) {
    try {
      ...
      // Try cache first
      const cacheKey = `subcategories_debug_${mainCategory}`;
      const cached = this.cache.get(cacheKey);
      if (cached) {
        return JSON.parse(cached);
      }
      ...
      // Cache results
      this.cache.put(cacheKey, JSON.stringify(uniqueSubcategories), CATEGORY_FILTER_CONFIG_DEBUG.CACHE_DURATION);
      ...
      throw new CategoryFilterErrorDebug(`Failed to get subcategories: ${error.message}`, 'GET_SUBCATEGORIES_FAILED');
    }
  }
  ...
  filterByCategory(mainCategory, subcategory = null, page = 1) {
    try {
      ...
      if (lastRow < 2) {
        return {
          ...
          pagination: {
            ...
            totalPages: 0,
            totalResults: 0,
            resultsPerPage: CATEGORY_FILTER_CONFIG_DEBUG.MAX_RESULTS_PER_PAGE
          }
        };
      }
      ...
      const totalPages = Math.ceil(totalResults / CATEGORY_FILTER_CONFIG_DEBUG.MAX_RESULTS_PER_PAGE);
      const startIndex = (page - 1) * CATEGORY_FILTER_CONFIG_DEBUG.MAX_RESULTS_PER_PAGE;
      const endIndex = startIndex + CATEGORY_FILTER_CONFIG_DEBUG.MAX_RESULTS_PER_PAGE;
      ...
      throw new CategoryFilterErrorDebug(`Filter operation failed: ${error.message}`, 'FILTER_FAILED');
    }
  }
  ...
  clearCache() {
    try {
      this.cache.remove('main_categories_debug');
      
      // Clear subcategory caches
      const categories = this.getMainCategories();
      categories.forEach(cat => {
        this.cache.remove(`subcategories_debug_${cat}`);
      });
...
class CategorySessionLoggerDebug {
  ...
  log(action, message, details = {}) {
    // ⚠️ KNOWN BUG: CFG is undefined - use CATEGORY_FILTER_CONFIG_DEBUG.DEBUG_MODE
    // This line demonstrates the original bug:
    // if (CFG && CFG.DEBUG_MODE) { ... }
    
    // FIXED VERSION:
    if (CATEGORY_FILTER_CONFIG_DEBUG.DEBUG_MODE) {
      const logEntry = {
...
// ============================================================================
// DEBUG API Functions - For testing/comparison only
// ============================================================================

/**
 * DEBUG: Get all main categories for filter dropdown
 */
function getMainCategoriesAPI_Debug() {
  try {
    const manager = new CategoryFilterManagerDebug();
    const categories = manager.getMainCategories();
    return {
      success: true,
      data: categories,
      _debug: 'Using DEBUG v2.0 manager'
    };
  } catch (error) {
    return {
      success: false,
 ...
 */
function filterByCategoryAPI_Debug(mainCategory, subcategory = null, page = 1) {
  try {
    const manager = new CategoryFilterManagerDebug();
...
/**
 * DEBUG: Clear all category caches
 */
function clearCategoryCacheAPI_Debug() {
  try {
    const manager = new CategoryFilterManagerDebug();
    manager.clearCache();
    return {
      success: true,
      message: 'Category cache cleared successfully (DEBUG)',
      _debug: 'Using DEBUG v2.0 manager'
    };
  } catch (error) {
    return {
